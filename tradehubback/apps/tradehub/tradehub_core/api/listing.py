...
def get_listings(
    ...
    fields = [
        ...
        "supplier_profile", "supplier_display_name",
...
def get_listing_detail(listing_id):
    ...
    supplier_data = None
    if listing.supplier_profile:
        try:
            supplier = frappe.get_doc("Supplier Profile", listing.supplier_profile)
            supplier_data = {
                "name": supplier.supplier_name or supplier.company_name,
                "companyName": supplier.company_name,
                "verified": bool(supplier.is_verified),
                "verificationType": supplier.verification_type,
                "yearsInBusiness": supplier.years_in_business or 0,
                "country": supplier.country,
                "city": supplier.city,
                "logo": supplier.logo,
                "responseTime": supplier.response_time,
                "responseRate": supplier.response_rate or 0,
                "onTimeDelivery": supplier.on_time_delivery or 0,
                "mainProducts": (supplier.main_products or "").split(",") if supplier.main_products else [],
                "employees": supplier.employees,
                "annualRevenue": supplier.annual_revenue,
                "certifications": (supplier.certifications or "").split(",") if supplier.certifications else [],
                "rating": supplier.average_rating or 0,
                "reviewCount": supplier.review_count or 0,
            }
        except Exception:
            pass
...
def _format_listing_card(listing):
    ...
    if listing.get("supplier_profile"):
        try:
            sp = frappe.db.get_value(
                "Supplier Profile",
                listing.supplier_profile,
                ["years_in_business", "country", "is_verified", "average_rating", "review_count"],
                as_dict=True,
            )
            if sp:
                supplier_years = sp.years_in_business or 0
                supplier_country = _get_country_code(sp.country) if sp.country else ""
                supplier_verified = bool(sp.is_verified)
                supplier_rating = sp.average_rating or 0
