...
def get_sellers(page=1, page_size=12, search=None):
    """
    Tüm aktif satıcıları listeler.

    GET /api/method/tradehub_core.api.seller.get_sellers
    """
...
list_sellers = get_sellers


@frappe.whitelist(allow_guest=True)
def get_seller(slug=None):
    """
    Tek bir satıcının halka açık profili.

    GET /api/method/tradehub_core.api.seller.get_seller?slug=SLR-XXXX
    """
    if not slug:
        frappe.throw(_("Satıcı kodu gereklidir"))
...
def get_public_profile(seller_code):
    """Alias for get_seller with seller_code param."""
    return get_seller(slug=seller_code)
